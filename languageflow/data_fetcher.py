...
from languageflow.datasets import REPO, Dataset
...
class DataFetcher:
    ...
    @staticmethod
    def download_data(data, url=None):
        if data not in REPO:
        ...
        dataset = Dataset(name=data,
                          type=REPO[data]["type"],
                          year=REPO[data]["year"])
        filepath = Path(CACHE_ROOT) / "datasets" / data / REPO[data]["filepath"]
        if Path(filepath).exists():
            print(f"Data is already existed: '{data}' in {filepath}")
            return
        cache_dir = ""

        datasets = {"VNESES", "VNTQ_SMALL", "VNTQ_BIG", "VNTC",
                    "VLSP2013_WTK", "VLSP2013_POS", "VTB_CHUNK", "VLSP2016_NER",
                    "VLSP2018_NER", "AIVIVN2019_SA", "VLSP2016_SA", "VLSP2018_SA",
                    "UTS2017_BANK"}
        if data in datasets:
            dataset = Dataset.get(data)
            if (dataset.license == "Closed") and (not url):
                print(f"\n{MISS_URL_ERROR}")
                return
    ...
    def list(all):
        ...
        for key in REPO:
            ...
            datasets.append([dataset.name, dataset.type, license, dataset.year, status])

        print(tabulate(datasets,
                       headers=["Name", "Type", "License", "Year", "Status"],
                       tablefmt='orgtbl'))
    ...
    def remove(data):
        if data not in REPO:
            print(f"No matching distribution found for '{data}'")
            return
        cache_dir = Path(CACHE_ROOT) / "datasets" / data
        if cache_dir.is_dir():
